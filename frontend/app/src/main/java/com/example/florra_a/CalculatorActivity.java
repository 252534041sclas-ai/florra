...
public class CalculatorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Check if opened from ProductDetails
        if (getIntent().getBooleanExtra("fromProductDetails", false)) {
            btnSaveCalculation.setText("ADD TO QUOTE");
            // Optionally change icon
             btnSaveCalculation.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_request_quote, 0, 0, 0);
        }
    }
    ...
    private void saveCalculation() {
        if (getIntent().getBooleanExtra("fromProductDetails", false)) {
            // Calculate area again just to be sure (or get from UI)
            String areaText = tvCoverageArea.getText().toString().replace(" sq. ft", "");
            
            Intent intent = new Intent(CalculatorActivity.this, RequestQuotationActivity.class);
            // Pass forward all product details
